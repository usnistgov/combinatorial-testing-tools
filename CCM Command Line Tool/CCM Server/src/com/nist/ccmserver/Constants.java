...
public class Constants {
	...
	public IntegerExpressionVariable mk_op(String op, IntegerExpressionVariable oprnd1, IntegerExpressionVariable oprnd2 ){		
		...
		if(op.equalsIgnoreCase(PLUS))	return Choco.plus(oprnd1, oprnd2);
		if(op.equalsIgnoreCase(MUL))	return Choco.mult(oprnd1, oprnd2);
		if(op.equalsIgnoreCase(MINUS))	return Choco.minus( oprnd1, oprnd2);
		if(op.equalsIgnoreCase(DIV))	return Choco.div( oprnd1, oprnd2);
		if(op.equalsIgnoreCase(MOD))	return Choco.mod( oprnd1, oprnd2);
		return null; 
	}	

	public Constraint mk_comp(String op, IntegerExpressionVariable oprnd1, IntegerExpressionVariable oprnd2 ){
		
		if(op.equalsIgnoreCase(EQ)||op.equalsIgnoreCase(EQ2)) 	return Choco.eq(oprnd1, oprnd2);
		if(op.equalsIgnoreCase(NEQ)) 	return Choco.neq(oprnd1, oprnd2);
		if(op.equalsIgnoreCase(LT)) 	return Choco.lt(oprnd1, oprnd2);
		if(op.equalsIgnoreCase(LE))		return Choco.leq(oprnd1, oprnd2);
		if(op.equalsIgnoreCase(GT))		return Choco.gt(oprnd1, oprnd2);
		if(op.equalsIgnoreCase(GE))		return Choco.geq(oprnd1, oprnd2);
		return null; 
	}	

	public Constraint mk_cst(String op, Constraint oprnd1, Constraint oprnd2 ){	
		
		if(op.equalsIgnoreCase(NOT))	return Choco.not(oprnd2);
		if(op.equalsIgnoreCase(AND))	return Choco.and(oprnd1, oprnd2);
		if(op.equalsIgnoreCase(OR)) 	return Choco.or( oprnd1, oprnd2);
		if(op.equalsIgnoreCase(IMP))	return Choco.implies(oprnd1, oprnd2);
		return null; 
	}
