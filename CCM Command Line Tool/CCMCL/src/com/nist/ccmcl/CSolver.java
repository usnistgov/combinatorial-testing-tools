...
public class CSolver {
     ...
     private void CreateCombinationsVariables(List<String[]> listParams)
     {
         ...
         for (String[] parm : listParams)
         {
            ...
            Parameter p = (Parameter) parameters.get(parm[0]);
             ...
             else if (p.getType() == Parameter.PARAM_TYPE_ENUM) //for enum type is the index from the general list of enum values created above
             {
            	 if (!parm[1].equals(""))
            	 {
            	 ...
            	 if (!values.isEmpty())
            		 chocoVariables.add(new ChocoVariable(p, values));//add to the chocoVariable list all parameters with their values
             }
         }
     ...
     public void CreateConstraintsForRandomTests()
     {
    	//CONSTRAINT
         ...
         varInUse = new java.util.LinkedHashSet();
             ...
             ConstraintChocoParser parser = new ConstraintChocoParser(i.get_cons(), parameters, new Constants(enumList, varInUse, chocoVariables));
             varInUse.clear();
             try
             {
     ...
     public void CreateConstraints(meConstraint cons)
     {
         ...
         varInUse = new java.util.LinkedHashSet();
             ...
             //create parser object										
             ConstraintChocoParser parser = new ConstraintChocoParser(cons.get_cons(), parameters, new Constants(enumList, varInUse, chocoVariables));
             varInUse.clear();
             try
             {
            	 //change constraint to valid format constraint for choco
                 ...
                 line="";
                  ...
                  for (int j = 0; j < x.length; j++)
                  {
                      ...
                      if (p.getType() == Parameter.PARAM_TYPE_BOOL)
                    	  line= line + (x[j].equals("0") ? Boolean.toString(false).toUpperCase() : Boolean.toString(true).toUpperCase()) + ",";
     ...
     public boolean Solver(JFrame screen,JProgressBar pb)
     {
        ...
        // for (meConstraint mc : lc)
         //{
            ...
            // ISolutionPool pool = s.getSearchStrategy().getSolutionPool();
            // 
             if (!s.isFeasible()) {JOptionPane.showMessageDialog(screen, "Constraints don't have a feasible solution!!"); return false;}
            	  ...
            	  while (variables.hasNext())
                  {
                      ...
                      t.add(value);
                       ...
                       p = lp.get(t.size()-1);
                          ...
                          if (p.getType() == Parameter.PARAM_TYPE_BOOL)
                         	 valueMap =  value==0 ? Boolean.toString(false).toUpperCase() : Boolean.toString(true).toUpperCase();
