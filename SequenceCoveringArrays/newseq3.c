...
while (m<NSEQ) {
   ...
   for (j=N-1; j>=0; j--) {
   for (k=N-1; k>=0; k--) {
      if (i != j && i != k && j != k) {seq[m][0]=i; seq[m][1]=j; seq[m][2]=k; m++; chk[i][j][k] = 0;}
      }}}
}
...
while ( !allcovered() && nt < MAXT) {
	...
	for (m=0; m<NTRIALS; m++) {
		...
		for (j=i+1; j<N-1; j++) {
		for (k=j+1; k<N; k++) {
			i1= tmptest[m][i]; j1= tmptest[m][j];  k1= tmptest[m][k];
			if ( chk[i1][j1][k1] == 0  && i != j && i != k && j != k) { cnt++; }
	}}}	
...
for (m=0; m<tst; m++) {
   ...
   for (k=j+1; k<N; k++) { 
   {
      if (chk[test[m][i]][test[m][j]][test[m][k]] == 0) ncov++;
	  chk[test[m][i]][test[m][j]][test[m][k]] = 1; 
...
for (i=0; i<N; i++) {
   for (j=0; j<N; j++) {
   for (k=0; k<N; k++) {
		if ( chk[i][j][k] == 1  && i != j && i != k && j != k) {cnt++; }
		}}}
